...
#[test]
fn exposes_webpack_require_within_the_same_eval_call() {
    let runtime = RomRuntime::new(RuntimeConfig::default()).unwrap();
    let result = runtime
        .eval_async_as_string(
            r#"
            (async () => {
                eval(`
                    !function() {
                        "use strict";
                        var c = function webpackRequire(id) { return c.m[id](); };
                        c.m = {
                            42: function() { return "answer"; },
                        };
                        var n;
                        var loaded = {};
                        var t = function(push, chunk) {
