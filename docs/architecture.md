...
- `crypto.getRandomValues()`, `crypto.randomUUID()`, `crypto.subtle.digest()`, HMAC flows, `AES-CTR` / `AES-CBC` / `AES-GCM` with browser-like parameter validation and `AES-GCM` tag lengths `96..128`, `AES-KW` wrapping flows, `PBKDF2` / `HKDF` derivation with browser-like parameter validation, and current `raw` / `jwk` import/export flows with browser-like edge validation are present
