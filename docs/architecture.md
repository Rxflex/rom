...
- `crypto.getRandomValues()`, `crypto.randomUUID()`, `crypto.subtle.digest()`, HMAC flows, `AES-GCM`, and `PBKDF2` derivation are present
...
- deeper networking and worker semantics still need work: fuller stream semantics, richer URL/file semantics, `no-cors`/opaque details, worker isolation fidelity, and broader `SubtleCrypto` coverage beyond the current digest/HMAC/AES-GCM/PBKDF2 slice
