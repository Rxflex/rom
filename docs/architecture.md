...
- `crypto.getRandomValues()`, `crypto.randomUUID()`, `crypto.subtle.digest()`, HMAC flows, `AES-CTR` / `AES-CBC` / `AES-GCM` with browser-like parameter validation and `AES-GCM` tag lengths `96..128`, `AES-KW` wrapping flows, `PBKDF2` / `HKDF` derivation with browser-like parameter validation, and secret-key wrap/unwrap flows are present
