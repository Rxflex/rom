...
- `crypto.getRandomValues()`, `crypto.randomUUID()`, `crypto.subtle.digest()`, HMAC flows, `AES-GCM`, `AES-KW` wrapping flows, `PBKDF2` derivation, `HKDF` derivation, and secret-key wrap/unwrap flows are present
...
- deeper networking and worker semantics still need work: fuller stream semantics, longer-lived `WebSocket` behavior, richer URL/file semantics, `no-cors`/opaque details, worker isolation fidelity, richer permission/device behavior, and broader `SubtleCrypto` coverage beyond the current digest/HMAC/AES-GCM/AES-KW/PBKDF2/HKDF slice
